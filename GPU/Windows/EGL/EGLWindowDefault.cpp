    ...
    switch (GetSurfaceType())
    {
        case GLSurfaceType::WINDOWS:
            ...
            if (surface == EGL_NO_SURFACE)
            {
                ...
                EGL_LOG_ERROR << "Create window surface fail, error is: " << EGLErrorToStr(error);
            }
            break;
        case GLSurfaceType::PBUFFER:
            ...
            {
                EGLint error = eglGetError();
                EGL_LOG_ERROR << "Create pbuffer surface fail, error is: " << EGLErrorToStr(error);
            }
            break;
        case GLSurfaceType::PIXMAP:
            assert(false);
            EGL_LOG_ERROR << "Not support surface type, surface type is: " << GLSurfaceType::PIXMAP;
            break;
        ...
        default:
            assert(false);
            EGL_LOG_ERROR << "Unknown surface type, maybe need to update code";
            break;
    }
    return surface;
}
